...
class TestParsing:
    def test_reads_objects_without_size_element(self, tmp_path, voc_xml):
        """IR-LPR annotations carry no <size>; the size must come from the image."""
        xml = voc_xml(CHAR_OBJECTS, tmp_path / "a.xml")
        boxes, size = parse_xml(xml)
        assert len(boxes) == 5
        assert size is None

    def test_reads_size_when_present(self, tmp_path, voc_xml):
        xml = voc_xml(CHAR_OBJECTS, tmp_path / "a.xml", size=(580, 317))
        _, size = parse_xml(xml)
        assert size == (580, 317)

    def test_malformed_xml_returns_empty(self, tmp_path, voc_xml):
        bad = tmp_path / "bad.xml"
        bad.write_text("<annotation><object>", encoding="utf-8")
        assert parse_xml(bad) == ([], None)

    def test_missing_file_returns_empty(self, tmp_path, voc_xml):
        assert parse_xml(tmp_path / "nope.xml") == ([], None)

    def test_objects_with_bad_coordinates_are_skipped(self, tmp_path, voc_xml):
        xml = tmp_path / "a.xml"
        xml.write_text(
            "<annotation>"
    ...
    def test_find_xml_handles_both_cases(self, tmp_path, voc_xml):
        img = tmp_path / "a.jpg"
        img.write_bytes(b"x")
...
class TestPlateText:
    def test_reads_left_to_right_by_xmin(self, tmp_path, voc_xml):
        shuffled = [CHAR_OBJECTS[i] for i in (3, 0, 4, 2, 1)]
        xml = voc_xml(shuffled, tmp_path / "a.xml")
        assert extract_plate_from_xml(xml) == "12ز81"

    def test_ignores_the_whole_plate_object(self, tmp_path, voc_xml):
        """Car annotations carry a plate box alongside the characters."""
        objects = [("کل ناحیه پلاک", 50, 50, 340, 250)] + CHAR_OBJECTS
        xml = voc_xml(objects, tmp_path / "a.xml")
        assert extract_plate_from_xml(xml) == "12ز81"

    def test_too_short_returns_none(self, tmp_path, voc_xml):
        xml = voc_xml([("1", 1, 1, 2, 2)], tmp_path / "a.xml")
        assert extract_plate_from_xml(xml) is None

    def test_no_objects_returns_none(self, tmp_path, voc_xml):
        xml = voc_xml([], tmp_path / "a.xml")
        assert extract_plate_from_xml(xml) is None


class TestPlateRegions:
    def test_prefers_explicit_plate_box(self, tmp_path, voc_xml):
        objects = [("کل ناحیه پلاک", 627, 770, 780, 816)] + CHAR_OBJECTS
        regions, _, source = plate_regions(voc_xml(objects, tmp_path / "a.xml"))
        assert source == "explicit"
        assert regions == [(627.0, 770.0, 780.0, 816.0)]

    def test_returns_every_plate_in_the_frame(self, tmp_path, voc_xml):
    ...
    def test_falls_back_to_character_union(self, tmp_path, voc_xml):
        regions, _, source = plate_regions(voc_xml(CHAR_OBJECTS, tmp_path / "a.xml"))
        assert source == "union"
        assert len(regions) == 1
        x1, y1, x2, y2 = regions[0]
        assert x1 < 59 and y1 < 56 and x2 > 325 and y2 > 245  # padded outward

    def test_empty_annotation(self, tmp_path, voc_xml):
        regions, _, source = plate_regions(voc_xml([], tmp_path / "a.xml"))
...
class TestYoloLabels:
    def test_normalised_centre_and_size(self, tmp_path, voc_xml):
        out = tmp_path / "a.txt"
    ...
    def test_boxes_are_clipped_in_pixel_space(self, tmp_path, voc_xml):
    ...
    def test_every_box_stays_inside_the_image(self, tmp_path, voc_xml):
        out = tmp_path / "a.txt"
    ...
    def test_one_line_per_plate(self, tmp_path, voc_xml):
        out = tmp_path / "a.txt"
        write_yolo_label(out, [(10.0, 10.0, 50.0, 30.0), (100.0, 60.0, 160.0, 90.0)], (400, 200))
        assert len(out.read_text().strip().splitlines()) == 2

    def test_degenerate_boxes_dropped(self, tmp_path, voc_xml):
        out = tmp_path / "a.txt"
...
class TestImageSize:
    def test_reads_jpeg_header(self, tmp_path, voc_xml):
    ...
    def test_reads_png_header(self, tmp_path, voc_xml):
        cv2 = pytest.importorskip("cv2")
        np = pytest.importorskip("numpy")
        path = tmp_path / "a.png"
    ...
    def test_unreadable_file_returns_none(self, tmp_path, voc_xml):
        path = tmp_path / "a.jpg"
        path.write_bytes(b"not an image")
        assert image_size(path) is None

    def test_missing_file_returns_none(self, tmp_path, voc_xml):
